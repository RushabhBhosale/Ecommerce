...
export default function CartProduct({ product }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between space-y-4 sm:space-y-0 sm:space-x-4 hover:shadow-lg hover:border-opacity-50 border border-opacity-0 rounded-md p-4 bg-white">
      <Link href={`/products/${product.id}`} className="flex items-center group">
        ...
        <p className="font-semibold text-xl group-hover:underline ml-4 sm:ml-0">{product.name}</p>
      </Link>
      <div className="flex items-center">
        ...
        <p className="text-xl font-semibold mt-2 sm:mt-0">
          <XMarkIcon className="w-4 h-4 text-gray-500 mr-1 mb-1 inline-block" />
          {product.formattedPrice}
        </p>
        <button onClick={() => removeItem(product.id)} className="ml-4 mt-2 sm:mt-0 hover:text-red-500">
